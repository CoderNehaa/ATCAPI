...
userRouter.get("/auth/google/callback", passport_1.default.authenticate("google", { failureMessage: "Error occurred" }), (req, res, next) => {
    const user = req.user;
    req.logIn(user, (err) => {
        if (err)
            return next(err);
        ...
        res.redirect(`${config_1.config.FRONT_END_DOMAIN}/auth/social-logins/callback`);
    });
});
